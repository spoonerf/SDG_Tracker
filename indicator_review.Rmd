...
world <- clean_names(ne_countries(scale = "large", returnclass = "sf"))
...
# The age of the most recent indicator data
...
num_countries <- glob_inds %>% 
  ...
  group_by(goal,indicator,entity) %>% 
  filter(latest_data_year == max(latest_data_year) | is.na(latest_data_year) & code != "ATA") %>% 
  ungroup() %>% 
  ...
  geom_density_ridges(scale = 1.5, gradient_lwd = 1., alpha = 0.6, from = 0, to = 233, 
                      jittered_points = TRUE, position = position_points_jitter(width = 0.075, height = 0),point_shape = '|', point_size = 1.5, point_alpha = 1, alpha = 0.7) + 
  theme_fiona() +
  sdg_fill_scale + 
  theme(legend.position = "none")+
  ylab("Sustainable Development Goal")+
...
bins <- c(2012:2020)
pal <-  colorBin("YlOrRd", domain = num_ind_countries$sum_ind, bins = bins)
